var iDevice = require('../ios-device/plugins/idevice')
...
module.exports = function(options) {
  ...
  var ports = options.ports.slice(
    0
  , options.ports.length - options.ports.length % 6
  )
  ...
  // Track and manage devices
    ...
    idevice.on('add', filterDevice(function(device) {
      ...
      var register = new Promise(function(resolve) {
        ...
        heartTimer = setInterval(beat, options.heartbeatInterval)
      ...
      function spawn() {
        var allocatedPorts = ports.splice(0, 6)
      ...
      function stop() {
        if (worker) {
          ...
          idevice.remove(device.type,device.id)
          worker.cancel()
        }
      }
    ...
    idevice.on('change', filterDevice(function(device) {
      flippedTracker.emit(device.id, 'change', device)
    }))

    idevice.on('remove', filterDevice(function(device) {
  ...
  var ideviceTimer = setInterval(checkDevice, 10000);
